...
class FakeJobDetectorMLP:
    def __init__(self):
        """
        Initialize the Fake Job Detector with MLP classifier and TF-IDF vectorizer.
        Based on your research, MLP performs best with TF-IDF without SMOTE.
        """
        # Define the pipeline with optimal parameters from your research
    ...
    def train(self, job_postings, labels):
        """
        Train the model (MLP works best without SMOTE based on your research).
        ...
        # Train the pipeline directly (no SMOTE needed for MLP)
    ...
    def explain_prediction(self, job_posting):
        ...
        # MLP-specific explanations (limited but we can add some neural network insights)
        reasons.append("Neural network identified patterns similar to known scam job postings")
        
        # If we couldn't find specific reasons, add a generic explanation
        if len(reasons) < 3:
            reasons.append("Multiple subtle patterns that are common in fraudulent job postings")
            reasons.append("Unusual combination of job details that don't align with industry standards")
    ...
    def load_model(cls, filepath):
        ...
        Returns:
            FakeJobDetectorMLP: Loaded model
        """
